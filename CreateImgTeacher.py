...
def getScheduleAsImg(Schedule):
    ...
    for urok in Schedule:
        ...
        row = [int(urok["Урок"]),urok["Группа"], urok["Подгруппа"], prepod_lines, urok["Предмет"], kabinet_lines,urok["ИЗМЕНЕНИЕ"], GetUrokTime(datetime.datetime.strptime(urok["Дата"], "%Y-%m-%d"), urok["Урок"])]
        table_data.append(row)

        prev_urok = urok

    y = 30 * sum([max([len(row[i]) for i in range(1, 4)]) for row in table_data]) + len(table_data) + 1
    image = Image.new('RGB', (1920, y + 350), color=(255, 255, 255))
    ...
    cell_width = [60, 300, 50, 250, 250, 100, 100, 100]  # Ширина ячеек для каждого столбца
    cell_height = 30
